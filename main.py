...
class GNN:
    def __init__(self):
        ...
        self.G_ngrs = None
        self.pos_tokenized = []
        self.nlp = spacy.load("en_core_web_sm")  # Load spaCy NLP model for NER
        self.entities = []  # To store entities for each document
    ...
    def extract_entities(self):
        """ Extract entities from the original data using spaCy's NER """
        self.entities = []
        for doc in self.nlp.pipe(self.orig_data):
            self.entities.append([(ent.text, ent.label_) for ent in doc.ents])
    ...
    def build_graph(self, metric, threshold=0.00001):
        ...
        for i, entity_list in enumerate(self.entities):
            for entity, _ in entity_list:
                entity_node = f"Entity_{entity}_{i}"  # Unique name for entity nodes
                G.add_node(entity_node)
                G.add_edge(i, entity_node)
